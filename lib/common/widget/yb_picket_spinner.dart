...
class _TimePickerSpinnerState extends State<TimePickerSpinner> {
  ...
  int currentSelectedSecondIndex = 60;
  ...
  int _getMinuteCount(){
    return (12 / widget.minutesInterval).floor();
  }
  int _getSecondCount(){
    return (30 / widget.secondsInterval).floor();
  }
  ...
  DateTime getDateTime() {
    ...
    //text = (DateTime.now().year-_getSecondCount()+int.parse(text)+1).toString();
    int year = currentTime.year-_getSecondCount()+second+1;
    int month = hour==0?12:hour;
  ...
  void initState() {
    ...
    currentSelectedHourIndex = _getHourCount()+currentTime.month ;//+ _getHourCount();//(currentTime.hour % (widget.is24HourMode ? 24 : 12)) + _getHourCount();
    ...
    currentSelectedSecondIndex = _getSecondCount()+_getSecondCount()-1;//(currentTime.second / widget.secondsInterval).floor() + (isLoop(_getSecondCount()) ? _getSecondCount() : 1);
    ...
    Widget _spinner = NotificationListener<ScrollNotification>(
      ...
      child: new ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if(isLoop(max)){
            tempIndex =(index % max) * interval;
            text = (tempIndex).toString();
          }
          else if(index != 0 && index != max + 1){
            tempIndex = ((index - 1)  % max) * interval;
            text = (tempIndex).toString();
          }
          if(controller == hourController && text == '0'){
            text = '12';
          }
          ...
          if(controller == secondController){
            //Log.d("_getSecondCount()= ${_getSecondCount()} index = $tempIndex $max");
            text = (currentTime.year-_getSecondCount()+tempIndex+1).toString();
          }
          /*if(isYear){
            text = (DateTime.now().year-int.parse(text)).toString();
          }else{
            text = text.padLeft(2, '0');
            if(text == '00'){
              text = '12';
          ...
          return Container(
            ...
            child: Container(child:Column(mainAxisAlignment:MainAxisAlignment.spaceEvenly,children: <Widget>[
              Text(
                text,
